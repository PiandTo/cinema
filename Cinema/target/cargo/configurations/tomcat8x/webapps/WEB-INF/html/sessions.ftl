...
<div class="wrapper">
    ...
    <div class="inside-wrapper">
        <h2 class="text-primary">Add new session</h2>
        <#if session?? && noErrors??>
            Your submitted data <br>
            Session id: ${session.id?c}<br>
        <#elseif session??>
            <form action="/admin/panel/session" method="post">
        ...
        <#else >
            <form action="/admin/panel/session" method="post">
                <div class="form-group">
                    <label for="inputDateTime">Session Date and time</label>
                    <input type="datetime-local" name="dateTime" class="form-control" id="inputDateTime" aria-describedby="emailHelp" placeholder="Enter session Date and Time">
                </div>
                <div class="form-group">
                    <label for="inputHallId">Hall</label>
                    <select name="hall.hallId">
                ...
                <div class="form-group">
                    <label for="inputTicketCost">Ticket cost</label>
                    <input type="text" name="ticketCost" class="form-control" id="inputTicketCost" placeholder="Enter ticket cost">
                </div>
                <button type="submit" class="btn btn-primary">Add</button>
            </form>
        </#if>
